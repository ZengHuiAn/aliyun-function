...
function signString(source, secret) {
  const buff = crypto
    ...
    .update(source, "utf8")
    .digest();
    const str = new Buffer(buff, "binary").toString("base64");
    console.log(str);
    console.log(new Buffer(buff).toString("base64"));
    return str
}
...
class Client {
  ...
  buildHeaders() {
    ...
    const headers = {
      ...
      host: this.host,
      "user-agent": `Node.js(${process.version}) OS(${process.platform}/${
        process.arch
      }) SDK(${pkg.name}@v${pkg.version})`,
  ...
  static getSignature(
    ...
    var stringToSign = helper.composeStringToSign(
      method,
      path,
      headers,
      queries
